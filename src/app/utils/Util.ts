...
class Util {
  ...
  static mapStringToDate(date: string): Date {
    // 27/02/2023-11:22
    const validDate = date.slice(6,10) + date.slice(2,6) + date.slice(0,2) + date.slice(10,16)
  ...
  static formatDate(date?: string): string {
    ...
    return validDate.toString().slice(3,11) + validDate.toString().slice(15, 21);
  }

  static randomId(length:number): number {
    return Math.pow(10,length) + Math.random()*9*Math.pow(10,length-1);
  }
   /**
